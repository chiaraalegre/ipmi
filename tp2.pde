...
float posX; // animacion txt1
int textVert, stopFrame, stopTime; // texto 4 vertical
...
void setup() {
  ...
  textX2 = 800; // posicion del texto 2
  textVert = height;
  stopFrame = 0;
  stopTime = 180;
 
  text3PosY = 0; // posicion del texto 3
  variableColor = 0; // valor del color txt3
  textX = 0;
  Xtext = 0;
  Ytext = 0;
  velX = 1;
...
void draw() {
  ...
  if (pantallaActual == 1) {
    ...
    // Dibujar botón de reinicio en la pantalla final
    image(botonReinicio, reinicioBotonX, reinicioBotonY, reinicioBotonAncho, reinicioBotonAlto);
  }
}

void mousePressed() {
  if (pantallaActual == 8 && mouseX > reinicioBotonX && mouseX < reinicioBotonX + reinicioBotonAncho && 
